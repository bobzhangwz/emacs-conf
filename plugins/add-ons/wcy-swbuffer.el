...
;; (setq wcy-buffer-exclude-regexps   '("^ .*" "^\\*.*" "^\\..*"))
...
;;;###autoload
(defun wcy-display-buffer-list (buffer-list window)
  ...
  (set-window-buffer window (car buffer-list) t))

;;;###autoload
(defun wcy-switch-buffer (arg &optional window)
...
if ARG is negative, switch backword, otherwise forward."
  ...
  (let* ((cycle-function (if (> arg 0) 'wcy-cycly-list ;; cycle forward or backward
         ...
         key)
    (or (windowp window) (setq window (selected-window)))
    (wcy-display-buffer-list tmp-buffer-list window)
    (while (null exitflag)
      ;; read a key
...
;;        (setq last-input-char (aref key (1- (length key))))
        ...
        (cond
         ...
         ((equal func 'keyboard-quit)
          ...
          (setq exitflag 1))
         (t (setq exitflag t)))))
    ;; switch to the selected bufqfer.
    (let ((selected-buffer (car tmp-buffer-list)))
      (if (buffer-name selected-buffer)
          (set-window-buffer window selected-buffer)))
